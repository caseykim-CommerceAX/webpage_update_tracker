...
test("대시보드와 주요 관리 화면을 연다", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "HEAD/BODY 태그 변경 진단" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "태그 변경 상세" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "페이지별 최신 진단" })).toBeVisible();
...
test("모바일 화면에서 문서 전체 수평 스크롤이 생기지 않는다", async ({ page }) => {
  ...
  for (const path of ["/", "/targets", "/runs"]) {
